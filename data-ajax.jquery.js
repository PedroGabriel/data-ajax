...
dataajaxcss = (function () {
  ...
  cssHtml +=
    "." +
    CONFIG.alerta.padrao.classe +
    "{position: fixed;z-index: 99999;width: 100%;min-height: 40px;line-height: 38px;text-align: center;left: 0px;border: 0px;color: #FFF;font-size: 14px;font-weight: 700;}";
  cssHtml +=
    "." +
    CONFIG.alerta.padrao.classe +
    "{background-color:" +
    CONFIG.alerta.padrao.fundo +
    ";color:" +
    CONFIG.alerta.padrao.fonte +
    ";}";
  cssHtml +=
    "." +
    CONFIG.alerta.sucesso.classe +
    "{background-color:" +
    CONFIG.alerta.sucesso.fundo +
    ";color:" +
    CONFIG.alerta.sucesso.fonte +
    ";}";
  cssHtml +=
    "." +
    CONFIG.alerta.erro.classe +
    "{background-color:" +
    CONFIG.alerta.erro.fundo +
    ";color:" +
    CONFIG.alerta.erro.fonte +
    ";}";
    ...
    $.each($("[data-ajax-action]"), function (i, e) {
      $(this).on("click.dataajax", function () {
        ...
        if ($e.is("[data-ajax-callback]")) {
          ...
          callback =
            typeof window["callback_" + callback_name] !== "undefined" &&
            window["callback_" + callback_name] !== "undefined"
              ? window["callback_" + callback_name]
              : function (r) {};
        }
        ...
        if ($e.is("[data-ajax-div-carregando]"))
          alerta_carregando = $($e.attr("data-ajax-div-carregando"));
        if ($e.is("[data-ajax-div-sucesso]"))
          alerta_sucesso = $($e.attr("data-ajax-div-sucesso"));
        if ($e.is("[data-ajax-div-erro]"))
          alerta_erro = $($e.attr("data-ajax-div-erro"));

        var aindaSalvando = setTimeout(function () {
          if (aindaSalvando)
            alerta(
              null,
              CONFIG.ajax.carregando.mensagem,
              null,
              null,
              "proximo",
              null,
        ...
        if (
          $e.is(
            "[data-ajax-div-carregando],[data-ajax-div-sucesso],[data-ajax-div-erro]"
          )
        ) {
          alerta_div_top = null;
          if ($e.is("[data-ajax-div-carregando]"))
          ...
          if ($e.find("[data-ajax-carregando]").length)
            alerta_carregando = $($e).find("[data-ajax-carregando]");
          if ($e.find("[data-ajax-sucesso]").length)
            alerta_sucesso = $($e).find("[data-ajax-sucesso]");
          if ($e.find("[data-ajax-erro]").length)
            alerta_erro = $($e).find("[data-ajax-erro]");
        }
        ...
        ajax_form_json(
          ...
          function (data) {
            ...
            if (CONFIG.sweetalert) swal.close();

            if (
              CONFIG.sweetalert &&
              !$e.is(
                '[data-ajax-sem-mensagem="sucesso"],[data-ajax-sem-mensagem=""]'
              )
            ) {
            ...
            if (
              ...
              var _title =
                data && data.title ? data.title : CONFIG.ajax.sucesso.titulo;
              var _msg =
                data && data.msg ? data.msg : CONFIG.ajax.sucesso.mensagem;
              alerta(
                CONFIG.alerta.sucesso.classe,
                _title && _msg ? [_title, _msg] : _msg,
                null,
                null,
                CONFIG.ajax.sucesso.segundos * 1000,
                alerta_div_top,
                alerta_carregando,
                alerta_sucesso,
                alerta_erro,
            ...
            aindaSalvando = false;
            if (!$e.is("[data-ajax-once]")) $e.removeData("locked");
          },
          function (data) {
            clearTimeout(aindaSalvando);
            aindaSalvando = false;
            if (
              !$e.is(
                '[data-ajax-sem-mensagem="erro"],[data-ajax-sem-mensagem=""]'
              )
            ) {
              var _title =
                data && data.title ? data.title : CONFIG.ajax.erro.titulo;
              var _msg =
                data && data.msg ? data.msg : CONFIG.ajax.erro.mensagem;
              alerta(
                CONFIG.alerta.erro.classe,
                _title && _msg ? [_title, _msg] : _msg,
                null,
                null,
                CONFIG.ajax.erro.segundos * 1000,
                null,
                alerta_carregando,
                alerta_sucesso,
                alerta_erro,
            ...
            if ($e.data("locked")) $e.removeData("locked");
          }
        );
      });
    }).css("cursor", "pointer");
    ...
    $.each($("form[data-ajax]"), function (i, e) {
      ...
      $form.off("submit.dataajax").on("submit.dataajax", function () {
        ...
        if (CONFIG.input.erro.classe)
          $("." + CONFIG.input.erro.classe).removeClass(
            CONFIG.input.erro.classe
          );
        var adicionar_icone_erro = function (input) {
          if (CONFIG.input.erro.icone.caractere) {
            ...
            input
              .before(
                '<div class="input-icon-error cart-payment ' +
                  CONFIG.input.erro.icone.classe +
                  '"><div class="icon">' +
                  CONFIG.input.erro.icone.caractere +
                  "</div></div>"
              )
              .prev()
              .css("display", "flex");
          ...
          if ($input.is('[type="checkbox"]') && !$input.is(":checked")) {
            $input.before(
              '<input type="hidden" name="' +
                $input.attr("name") +
                '" value="" data-required data-ajax-auto-generated-validator>'
            );
          }
        ...
        $.each(data, function (name, value) {
          ...
          if ($input.is("[data-required]")) {
            ...
            var result = "";
            if (data_ajax_validation[$input.attr("data-required")]) {
              result = data_ajax_validation[$input.attr("data-required")](
                value
              );
              if (typeof result === typeof "") validator_result = false;
            }

            if (!value || !value.length || !validator_result) {
              if (typeof result === typeof "" && !last_validator_error) {
                last_validator_error = result;
              }
              if (debug) console.log(name, value, result);

              form_erro = true;
              $input.addClass(CONFIG.input.erro.classe);
              if ($input.closest("[data-ajax-validacao]").length)
                $input
                  .closest("[data-ajax-validacao]")
                  .addClass(CONFIG.input.erro.classe);
        ...
        $.each($("[data-required]:not([name])", $form), function (
          element,
          index
        ) {
          var $input = $(this, $form);
          ...
          var result = "";
          if (data_ajax_validation[$input.attr("data-required")]) {
            result = data_ajax_validation[$input.attr("data-required")](value);
            if (typeof result === typeof "") validator_result = false;
          }
          if (!value || !value.length || !validator_result) {
            if (typeof result === typeof "" && !last_validator_error)
        ...
        if ($form.is("[data-ajax-callback]")) {
          var callback = $form
            .attr("data-ajax-callback")
            .replace(/^callback_/, "");
          var before_callback =
            typeof window["callback_" + callback] !== "undefined" &&
            window["callback_" + callback].before !== "undefined"
              ? window["callback_" + callback].before
              : function (r) {};
        }

        var _alerta_erro = {
          titulo: "",
          mensagem: last_validator_error ? last_validator_error : "",
        };
        var formData = new FormData($($form)[0]);
        var cb = before_callback($form, formData, markInput, _alerta_erro);
        if (cb === false || cb === 0) form_erro = true;
        else if (cb) formData = cb;
        ...
        if ($form.is("[data-ajax-callback]")) {
          ...
          var sucesso_callback =
            typeof window["callback_" + callback] !== "undefined" &&
            window["callback_" + callback].sucesso !== "undefined"
              ? window["callback_" + callback].sucesso
              : function (r) {};
          var erro_callback =
            typeof window["callback_" + callback] !== "undefined" &&
            window["callback_" + callback].erro !== "undefined"
              ? window["callback_" + callback].erro
              : function (r) {};
          var input_erro_callback =
            typeof window["callback_" + callback] !== "undefined" &&
            window["callback_" + callback].input_erro !== "undefined"
              ? window["callback_" + callback].input_erro
              : function (r) {};

          var after_callback =
            typeof window["callback_" + callback] !== "undefined" &&
            window["callback_" + callback].after !== "undefined"
              ? window["callback_" + callback].after
              : function (r) {};
        } else {
        ...
        if (
          ...
          alerta_div_top = null;
          if ($form.is("[data-ajax-div-carregando]"))
            alerta_carregando = $($form.attr("data-ajax-div-carregando"));
          if ($form.is("[data-ajax-div-sucesso]"))
            alerta_sucesso = $($form.attr("data-ajax-div-sucesso"));
          if ($form.is("[data-ajax-div-erro]"))
            alerta_erro = $($form.attr("data-ajax-div-erro"));
        } else {
          if ($form.find("[data-ajax-carregando]").length)
            alerta_carregando = $($form).find("[data-ajax-carregando]");
          if ($form.find("[data-ajax-sucesso]").length)
            alerta_sucesso = $($form).find("[data-ajax-sucesso]");
          if ($form.find("[data-ajax-erro]").length)
            alerta_erro = $($form).find("[data-ajax-erro]");
        }
        ...
        if (!form_erro) {
          if ($form.is("[data-ajax-get]")) {
            if ($form.attr("data-ajax-get") == "stay") {
              window.history.pushState(
                "",
                "",
                [CONFIG.baseurl, location.pathname].join("") +
                  "?" +
                  $form.serialize()
              );
            } else {
              var action = $form.is("[action]")
                ? $form.attr("action")
                : $form.attr("data-ajax");
              window.location.href =
                CONFIG.baseurl + action + "?" + $form.serialize();
              return true;
            }
          }
          ...
          var lock_form = false;
          if ($form.is("[data-ajax-once]")) lock_form = true;
          ...
          ajax_form(
            ...
            function (data) {
              ...
              if (
                ...
                swal_callback = function () {
                  if (sucesso_callback)
                    sucesso_callback(data, $form, markInput);
              ...
              if (!lock_form) $form.removeData("locked");
            },
            function (data) {
              clearTimeout(aindaSalvando);
              aindaSalvando = false;
              ...
              if ($form.data("locked")) $form.removeData("locked");
          ...
          if (
            ...
            var _msg = $form.is("[data-ajax-erro-mensagem]")
              ? $form.is("[data-ajax-erro-mensagem]")
              : CONFIG.form.erro.mensagem;
            if (_alerta_erro !== false) {
              if (_alerta_erro.titulo.length) _title = _alerta_erro.titulo;
              if (_alerta_erro.mensagem.length) _msg = _alerta_erro.mensagem;
            }
...
function ajax_form(url, data, success, error, deep_error, upload, json) {
  ...
  deep_error =
    typeof deep_error == "undefined"
      ? function (r) {
          console.log("Deep ajax error or not json at " + url, data, r);
          error(r);
        }
      : deep_error;
  ...
  var config = {
    ...
    error: function (r, status) {
      if (upload && status == "timeout") r.msg = CONFIG.ajax.upload.timeout;
    ...
    success: function (data, status) {
      ...
      if (!data || !data.status) error(data);
      else success(data);
    },
  };
  if (!json) config.contentType = false;
        ...
        function (evt) {
          if (evt.lengthComputable) {
            ...
            if (
              ...
              $("#swal2-content", swal.getContent()).html(
                percentComplete + "%"
              );
          }
        },
...
function alerta(
  ...
  div_carregando =
    typeof div_carregando === "undefined" ? null : div_carregando;
  div_sucesso = typeof div_sucesso === "undefined" ? null : div_sucesso;
  div_erro = typeof div_erro === "undefined" ? null : div_erro;

  ainda_carregando =
    typeof ainda_carregando === "undefined" ? false : ainda_carregando;
  ...
  if (!div_carregando && !div_sucesso && !div_erro) {
    if (CONFIG.sweetalert && typeof Swal === typeof function () {}) {
      var state = "info";
      if (classe == CONFIG.alerta.sucesso.classe) state = "success";
      else if (classe == CONFIG.alerta.erro.classe) state = "error";
      ...
      if (title) swal_conf.title = title;
    ...
    if (direction == "top") {
      var alerta = $(document.createElement("div"))
        ...
        .addClass(
          "" + CONFIG.alerta.padrao.classe + " " + (classe ? classe : "")
        )
        .text(mensagem ? mensagem : "")
        .attr("data-ajax-loading", "1")
        .appendTo("body")[0];
    ...
    if (div_top) {
      $(
        "[data-ajax-carregando],[data-ajax-sucesso],[data-ajax-erro]",
        div_top
      ).css(CONFIG.alerta_interno.invisivel);
      ...
      if (classe == CONFIG.alerta.padrao.classe) {
        ...
        if (div.attr("data-ajax-carregando"))
          div_mensagem = $(div.attr("data-ajax-carregando"));
      ...
      if (classe == CONFIG.alerta.sucesso.classe) {
        ...
        var div_mensagem = div;
        if (div.attr("data-ajax-sucesso"))
          div_mensagem = $(div.attr("data-ajax-sucesso"));
        div_mensagem.text(mensagem);
      }
      if (classe == CONFIG.alerta.erro.classe) {
        ...
        var div_mensagem = div;
        if (div.attr("data-ajax-erro"))
          div_mensagem = $(div.attr("data-ajax-erro"));
        div_mensagem.text(mensagem);
      }
    } else {
      ...
      if (classe == CONFIG.alerta.padrao.classe) {
        ...
        var div_mensagem = div;
        if (div.attr("data-ajax-div-texto"))
          div_mensagem = $(div.attr("data-ajax-div-texto"));
        div_mensagem.text(mensagem);
      }
      if (classe == CONFIG.alerta.sucesso.classe) {
        var div = div_sucesso;
...
$(function () {
  $.fn.formToData = function () {
    ...
    a = a.concat(
      $("input[type=checkbox]:not(:checked)", this)
        .map(function () {
...
var data_ajax_validation = {
  cpf: function (val) {
    val = val.toString().replace(/\D/g, "");
    var Soma;
    var Resto;
    Soma = 0;
    if (val == "00000000000") return "CPF inválido";
    ...
    if (Resto != parseInt(val.substring(9, 10))) return "CPF inválido";
    ...
    if (Resto == 10 || Resto == 11) Resto = 0;
    if (Resto != parseInt(val.substring(10, 11))) return "CPF inválido";
